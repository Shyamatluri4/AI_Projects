...
"""
AI Travel Agent – Multi-Agent LLM Travel Planner
Author: Syam Atluri
Description: AI-powered travel planning using LLM agents, real-time search, and itinerary generation.
...
st.title("AI Travel Agent – Multi-Agent LLM Planner")
...
if openai_api_key and serp_api_key:
    ...
    budget = st.number_input("What's your budget? (in USD)", min_value=0, value=1000, step=100)
    travel_style = st.selectbox("What's your travel style?", ["Budget", "Mid-range", "Luxury", "Adventure", "Cultural", "Relaxation"])
        ...
        if st.button("Generate Itinerary"):
            try:
                with st.spinner("Researching your destination..."):
                    ...
                    research_results: RunOutput = researcher.run(f"Research {destination} for a {num_days} day trip with a {budget} USD budget and {travel_style} travel style", stream=False)
                ...
                st.error(f"An error occurred while generating the itinerary: {str(e)}")
                st.session_state.itinerary = None
